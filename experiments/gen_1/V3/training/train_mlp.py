...
def load_data(config: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Get momentum: either from file or derive from q/p in X[:, 4]
    if 'P' in data:
        P = data['P']  # [N] - momentum
    else:
        # Derive momentum from q/p (column 4)
        qop = X[:, 4]  # q/p in 1/MeV
        P = np.abs(1.0 / qop)  # momentum in MeV
        print(f"  Derived P from q/p (range: {P.min():.0f} - {P.max():.0f} MeV)")
