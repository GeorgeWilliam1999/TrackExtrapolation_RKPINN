...
% ============================================================================
\section{NN Field Map Integration}\label{sec:nn_field_map}
% ============================================================================

This section documents a parallel line of investigation: replacing the
trilinear-interpolated magnetic field map inside the Runge--Kutta
extrapolator itself with a lightweight neural network.  Unlike the
full-trajectory NN extrapolators (V1--V4), this preserves the adaptive RK
loop and only modifies the field-evaluation kernel.

\subsection{Motivation}

Trilinear interpolation on the $81\times81\times146$ field grid requires
random memory accesses across an ${\sim}\SI{11}{MB}$ lookup table.
A compact NN (\num{227}~parameters, \num{416}~FLOPs) can replace these
lookups with arithmetic that stays in registers, potentially winning on
cache-miss-dominated workloads.

\subsection{Model Architecture}

A single-hidden-layer ReLU network is trained on the full field grid from
\texttt{twodip.rtf} (\num{957906}~grid points):

\begin{table}[H]
\centering
\caption{NN field map model specification.}
\label{tab:nn_field_spec}
\begin{tabular}{@{} l l @{}}
\toprule
Property & Value \\
\midrule
Input    & $(x, y, z)$ in mm \\
Output   & $(B_x, B_y, B_z)$ in Gauss \\
Hidden layers & 1 $\times$ 32 neurons \\
Activation & ReLU \\
Parameters & 227 \\
FLOPs per eval & 416 \\
Training epochs & 500 (patience 50) \\
Training strategy & log-space weight schedule \\
\bottomrule
\end{tabular}
\end{table}
...
The field map NN is integrated via a runtime property on
\texttt{TrackFieldExtrapolatorBase}:

\begin{verbatim}
  Gaudi::Property<std::string> UseNNFieldMap = "none";
  // Options: "none", "scalar_silu", "scalar_relu", "avx2_relu"
\end{verbatim}

Three inference backends are available:
\begin{description}[nosep]
  \item[\texttt{scalar\_silu}] Scalar C++ with SiLU activation
    (\texttt{FieldMapNNWeights.h}).
  \item[\texttt{scalar\_relu}] Scalar C++ with ReLU activation
    (\texttt{FieldMapNNReLUWeights.h}).
  \item[\texttt{avx2\_relu}] AVX2 vectorised ReLU inference using
    \texttt{\_mm256\_fmadd\_ps} with pre-transposed weights for contiguous
    SIMD loads.  Evaluates one field point per call using 8-wide
    \texttt{float32} lanes.
\end{description}

A batched template \texttt{evaluate\_relu\_avx2\_batch<N>()} is also
provided for future track-level batching, where $N$~field evaluations
execute in lock-step with a single pass through the weight arrays.

\subsection{Sub-Operation Timing Framework}

To measure \emph{where} time is spent inside each RK propagation, a
cycle-level timing framework was added:

\begin{table}[H]
\centering
\caption{Sub-operation timing channels.}
\label{tab:subtimers}
\begin{tabular}{@{} l l @{}}
\toprule
Channel & What is timed \\
\midrule
\texttt{field\_cycles}    & All \texttt{fieldVector()} calls (6 per CK step) \\
\texttt{deriv\_cycles}    & \texttt{evaluateDerivatives()} \\
\texttt{butcher\_cycles}  & Butcher tableau stage accumulation \\
\texttt{jacobian\_cycles} & \texttt{evaluateStepJacobian()} \\
\texttt{stepsize\_cycles} & Adaptive step-size control \\
\texttt{total\_cycles}    & Entire propagation \\
\texttt{nsteps}           & Accepted RK steps \\
\texttt{nrejected}        & Rejected steps \\
\bottomrule
\end{tabular}
\end{table}

Timing uses \texttt{\_\_rdtsc()} (x86-64 cycle counter).  The framework
is gated by a property \texttt{EnableSubTimers = false} and has zero
overhead when disabled (no \texttt{rdtsc} calls, no branches).  Results
are exposed via a \texttt{thread\_local} struct and virtual accessors on
\texttt{TrackFieldExtrapolatorBase}, allowing \texttt{TesterSOA} to write
per-track timing columns into ROOT ntuples.

\subsection{Benchmark Infrastructure}

A dedicated benchmark configuration (\texttt{benchmark\_nn\_field\_map.py})
runs four variants in a single job:

\begin{enumerate}[nosep]
  \item \textbf{Trilinear} --- baseline \texttt{UseNNFieldMap = "none"}
  \item \textbf{NN SiLU} --- \texttt{UseNNFieldMap = "scalar\_silu"}
  \item \textbf{NN ReLU (scalar)} --- \texttt{UseNNFieldMap = "scalar\_relu"}
  \item \textbf{NN ReLU (AVX2)} --- \texttt{UseNNFieldMap = "avx2\_relu"}
\end{enumerate}

All four use Cash--Karp (RK4/5) with \texttt{EnableSubTimers = True},
propagating from $z = \SI{3000}{mm}$ to
$z = \SI{12000}{mm}$.  Sub-timer columns in the output ROOT file
enable direct comparison of field-evaluation cost across backends.

An analysis notebook
(\texttt{nn\_field\_map\_benchmark\_analysis.ipynb}) provides:
\begin{itemize}[nosep]
  \item Total wall time comparison (box plots, per variant)
  \item Stacked sub-operation breakdown
  \item Per-step field evaluation cost
  \item Position/slope residuals vs.\ trilinear ground truth
  \item Amdahl's-law projection (theoretical speed-up vs.\ field fraction)
  \item Timing vs.\ momentum ($1/p$) correlation
  \item Auto-generated \LaTeX{} summary tables
\end{itemize}

\subsection{Per-Field-Call Speed Results}

The benchmark was run with four variants on a synthetic grid of
$11^3 = \num{1331}$ test tracks, propagating from $z = \SI{3000}{mm}$ to
$z = \SI{12000}{mm}$ with Cash--Karp (RK4/5) adaptive stepping.
Sub-operation cycles are measured via \texttt{\_\_rdtsc()} and normalised
per \texttt{fieldVector()} call (6~calls per CK stage attempt).

\begin{table}[H]
\centering
\caption{Per-field-call speed comparison (RDTSC cycles).}
...
NN SiLU          & 300 & 330 & $0.23\times$ & 70.3\% \\
NN ReLU (scalar) &  67 &  72 & $1.05\times$ & 33.6\% \\
NN ReLU (AVX2)   &  43 &  49 & $1.62\times$ & 24.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
Key observations:
\begin{itemize}[nosep]
  \item \textbf{SiLU is $4.3\times$ slower} than trilinear per field call.
    The \texttt{std::exp()} in $x \cdot \sigma(x)$ dominates; the
    \num{512}~FLOPs for SiLU translate to ${\sim}300$~cycles.
  \item \textbf{Scalar ReLU matches trilinear} at ${\sim}70$~cycles/call.
    The \num{416}~FLOPs of register-only arithmetic (no memory lookups)
    are competitive with trilinear's cache-dependent interpolation.
  \item \textbf{AVX2 ReLU is $1.62\times$ faster} at 43~cycles/call,
    using \texttt{\_mm256\_fmadd\_ps} with pre-transposed weights and
    fused output accumulation (no intermediate hidden-layer store).
  \item Field evaluation accounts for only ${\sim}17\%$ of total RK step
    cost (trilinear baseline).  By Amdahl's law, even a $10\times$
    field speedup yields at most ${\sim}1.2\times$ total propagation
    speedup.
\end{itemize}

\begin{table}[H]
\centering
\caption{Sub-operation cost breakdown per RK trial step (mean cycles).}
\label{tab:sub_op_breakdown}
\begin{tabular}{@{} l r r r r @{}}
\toprule
Sub-operation & Trilinear & NN SiLU & NN ReLU & NN AVX2 ReLU \\
\midrule
Field eval       &  420 (16.9\%) & 1982 (70.3\%) & 431 (33.6\%) & 292 (24.0\%) \\
Derivatives      &  193 (9.6\%)  &   96 (3.4\%)  &  94 (7.7\%)  &  95 (9.1\%) \\
Butcher accum    &  192 (9.5\%)  &  127 (3.9\%)  & 119 (8.6\%)  & 119 (9.8\%) \\
Jacobian         &  594 (28.5\%) &   56 (7.9\%)  &  56 (17.8\%) &  57 (20.4\%) \\
Step-size ctrl   &   64 (2.7\%)  &    3 (0.5\%)  &   4 (1.2\%)  &   3 (1.2\%) \\
Other/overhead   &  656 (32.8\%) &  523 (14.0\%) & 511 (31.1\%) & 513 (35.5\%) \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Known limitation.}
The NN field map currently returns wrong-magnitude values in the C++
integration (a units mismatch between the training data normalisation and
the LHCb Gauss convention consumed by the RK derivatives).  This causes
the adaptive step-size controller to accept one giant step across the
full $z$-range (${\sim}0$~accepted steps vs.\ ${\sim}11$ for trilinear).
The per-call RDTSC measurements are nevertheless valid---they measure
\texttt{fieldVector()} cost independently of the step count.  Total
propagation time comparisons will become meaningful once the units issue
is resolved.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{PINNs achieve excellent slopes} (\num{0.0003} vs.\
    ...
    capacity.
  \item \textbf{AVX2 NN field map is $1.6\times$ faster} per
    \texttt{fieldVector()} call than trilinear interpolation
    (43~vs.~70~RDTSC cycles), validating the register-only arithmetic
    approach for field evaluation (\S\ref{sec:nn_field_map}).
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item SiLU activation for NN field map ($4.3\times$ slower than
    trilinear per call due to \texttt{std::exp()}).
  \item NN field map units: training data normalisation does not match
    the Gauss convention expected by the RK derivatives, causing wrong
    step counts.  Per-call speed measurements are valid; total
    propagation comparisons await a fix.
\end{enumerate}
...
Medium & SIMD/batched C++ (\S\ref{sec:nn_field_map}) & $2$--$50\times$ speed \checkmark \\
...
{\small\itshape Document generated: March 12, 2026.\\
Source: V1--V4 documentation, CSV results, benchmark data, and analysis
notebooks in \texttt{experiments/gen\_1/}.}
