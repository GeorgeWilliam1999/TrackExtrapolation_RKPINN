...
private:
  ...
  void extrapolate( const LHCb::Magnet::MagneticFieldGrid* grid, double& zIn, double pIn[5], double& zNew,
                    double pOut[5], double fQp[25], int& istat ) const;

  /// Interface to 4th order Runga-Kutta
  void rk4order( const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, double* p_in, double& z_out, double* p_out,
                 double* rkd, int& ierror ) const;

  /// Interface to fast 4th order Runga-Kutta
  void rk4fast( const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, double* p_in, double& z_out, double* p_out,
                double* rkd, int& ierror ) const;

  /// Interface to 5th order Runga-Kutta
  void rk5order( const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, double* p_in, const double& error,
                 double& z_out, double* p_out, double* rkd, int& ierror ) const;

  /// Interface to fast 5th order Runga-Kutta
  void rk5fast( const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, double* p_in, const double& error,
                double& z_out, double* p_out, double* rkd, int& ierror ) const;

  /// interface to 5th order with derivatives caculated by numerical derivatives
  void rk5numde( const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, double* p_in, const double& error,
                 double& z_out, double* p_out, double* rkd, int& ierror ) const;

  /// Without derivatives rkd and ierror flag
  void rk5fast( const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, double* p_in, const double& error,
                double& z_out, double* p_out ) const;
  ...
  double m_stepMin    = 200. * Gaudi::Units::mm; ///<
  double m_stepMinRK5 = 20. * Gaudi::Units::mm;  ///<
};

DECLARE_COMPONENT( TrackHerabExtrapolator )
...
StatusCode TrackHerabExtrapolator::initialize() {
  info() << "ExtrapolatorID=" << m_extrapolatorID.value() << endmsg;
  return TrackFieldExtrapolatorBase::initialize();
}

// Propagate a state vector from zOld to zNew
// Transport matrix is calulated when transMat pointer is not NULL
StatusCode TrackHerabExtrapolator::propagate( Gaudi::TrackVector& stateVec, double zOld, double zNew,
                                              Gaudi::TrackMatrix* transMat, IGeometryInfo const&, LHCb::Tr::PID /*pid*/,
  ...
  // transport
  this->extrapolate( grid, zIn, pIn, zNew, pOut, fQp, istat );
  ...
  if ( transMat != NULL ) {
    ...
    for ( i = 0; i < 5; ++i ) {
      for ( int j = 0; j < 5; ++j ) { ( *transMat )( i, j ) = fQp[( 5 * j ) + i]; }
    }
  }
...
// Interface to Hera-b code
//===========================================================================
void TrackHerabExtrapolator::extrapolate( const LHCb::Magnet::MagneticFieldGrid* grid, double& zIn, double pIn[5],
                                          double& zNew, double pOut[5], double fQp[25], int& istat ) const {
  switch ( m_extrapolatorID ) {

  case 1:
    rk4fast( grid, zIn, pIn, zNew, pOut, fQp, istat );
    break;

  case 2:
    rk4order( grid, zIn, pIn, zNew, pOut, fQp, istat );
    break;

  case 3:
    rk5fast( grid, zIn, pIn, m_error, zNew, pOut, fQp, istat );
    break;

  case 4:
    rk5order( grid, zIn, pIn, m_error, zNew, pOut, fQp, istat );
    break;

  case 5:
    rk5numde( grid, zIn, pIn, m_error, zNew, pOut, fQp, istat );
    break;

  default:
    Warning( "Incorrect Extrapolator name - taking rk5order !!!!", StatusCode::SUCCESS, 1 ).ignore();
    rk5order( grid, zIn, pIn, m_error, zNew, pOut, fQp, istat );
    break;
  }
}
...
void TrackHerabExtrapolator::rk5order( const LHCb::Magnet::MagneticFieldGrid* grid,
                                       double&                                z_in, // z value for input parameters
                                       double*       p_in,  // input track parameters (x,y,tx,ty,Q/p)
                                       const double& error, // acceptable errors in mm
...

{
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    debug() << format( "rk5order  zIn %8.1f zOut %8.1f X %7.1f y %7.1f ", z_in, z_out, p_in[0], p_in[1] ) << endmsg;
  }
  ...
  for ( step = 0; step < 6; ++step ) {
    ...
    txty      = tx * ty;
    tx2ty21   = 1.0 + tx2 + ty2;
    I_tx2ty21 = 1.0 / tx2ty21 * qp;
    tx2ty2    = sqrt( tx2ty21 );
    // double I_tx2ty2 = qp * hC / tx2ty2 ; // unused?
    tx2ty2 *= hC;
    tx2ty2qp = tx2ty2 * qp;

    Ax[step] = ( txty * mB.x() + ty * mB.z() - ( 1.0 + tx2 ) * mB.y() ) * tx2ty2;
    Ay[step] = ( -txty * mB.y() - tx * mB.z() + ( 1.0 + ty2 ) * mB.x() ) * tx2ty2;

    Ax_tx[step] = Ax[step] * tx * I_tx2ty21 + ( ty * mB.x() - 2. * tx * mB.y() ) * tx2ty2qp;
    Ax_ty[step] = Ax[step] * ty * I_tx2ty21 + ( tx * mB.x() + mB.z() ) * tx2ty2qp;
    Ay_tx[step] = Ay[step] * tx * I_tx2ty21 + ( -ty * mB.y() - mB.z() ) * tx2ty2qp;
    Ay_ty[step] = Ay[step] * ty * I_tx2ty21 + ( -tx * mB.y() + 2. * ty * mB.x() ) * tx2ty2qp;
  ...
  for ( i = 0; i < 4; ++i ) { p_out[i] = x0[i] + c[0] * k[i] + c[2] * k[8 + i] + c[3] * k[12 + i] + c[5] * k[20 + i]; }
  p_out[4] = p_in[4];
  ...
  //
  for ( i = 0; i < 2; ++i ) {
    p1_out[i] = x0[i] + c1[0] * k[i] + c1[2] * k[8 + i] + c1[3] * k[12 + i] + c1[4] * k[16 + i] + c1[5] * k[20 + i];
  }
  ...
  for ( step = 0; step < 6; ++step ) {
    for ( i = 0; i < 4; ++i ) {
      x[i]   = x0[i];
      step_j = ( ( step - 1 ) * step ) / 2 + 1;
      for ( j = 0; j < step; ++j ) { x[i] += b[step_j + j] * k1[j * 4 + i]; }
    }
    step4         = step * 4;
    k1[step4]     = x[2] * h;
    k1[step4 + 1] = x[3] * h;
  ...
  for ( i = 0; i < 4; ++i ) {
    if ( i != 2 ) { rkd[10 + i] = x0[i] + c[0] * k1[i] + c[2] * k1[8 + i] + c[3] * k1[12 + i] + c[5] * k1[20 + i]; }
  }
  ...
  if ( ( fabs( p1_out[0] - p_out[0] ) > error ) || ( fabs( p1_out[1] - p_out[1] ) > error ) ) {
    if ( fabs( h ) > m_stepMinRK5 ) {
      z2_out = z_in + 0.5 * h;
      rk5order( grid, z_in, p_in, error, z2_out, p2_out, rkd1, ierror );
      rk5order( grid, z2_out, p2_out, error, z_out, p_out, rkd2, ierror );
      ...
      rkd[18] = 1.;
      rkd[19] = 0.;
      rkd[20] = rkd1[20] + rkd2[10] * rkd1[22] + rkd2[15] * rkd1[23] + rkd2[20];
      rkd[21] = rkd1[21] + rkd2[11] * rkd1[22] + rkd2[16] * rkd1[23] + rkd2[21];
      rkd[22] = rkd1[22] + rkd2[17] * rkd1[23] + rkd2[22];
      rkd[23] = rkd2[13] * rkd1[22] + rkd1[23] + rkd2[23];
      rkd[24] = 1.;
    }
...
//-- Author : A.Spiridonov  26.03.98
//===========================================================================
void TrackHerabExtrapolator::rk5fast( const LHCb::Magnet::MagneticFieldGrid* grid,
                                      double&                                z_in, // z value for input parameters
                                      double*       p_in,  // input track parameters (x,y,tx,ty,Q/p)
                                      const double& error, // acceptable errors in mm
                                      double&       z_out, // z value for output parameters
...

{
  ...
  for ( step = 0; step < 6; ++step ) {
    x[0]   = 0.0;
    step_j = ( ( step - 1 ) * step ) / 2 + 1;
    for ( j = 0; j < step; ++j ) { x[0] += b[step_j + j] * k1[j * 4]; }
    x[2]   = 0.0;
    step_j = ( ( step - 1 ) * step ) / 2 + 1;
    for ( j = 0; j < step; ++j ) { x[2] += b[step_j + j] * k1[j * 4 + 2]; }
  ...
  if ( fabs( p1_out[0] - p_out[0] ) > error ) {
    ...
    if ( fabs( h ) > m_stepMin * 3. ) {
      z2_out = z_in + 0.5 * h;
      rk5fast( grid, z_in, p_in, error, z2_out, p2_out );
      rk5fast( grid, z2_out, p2_out, error, z_out, p_out );
    } else if ( fabs( h ) > m_stepMin ) {
      z2_out = z_in + 0.5 * h;
      rk4fast( grid, z_in, p_in, z2_out, p2_out );
      rk4fast( grid, z2_out, p2_out, z_out, p_out );
    }
  } // end of stepsize control
...
void TrackHerabExtrapolator::rk5fast(                          // Without derivatives
    const LHCb::Magnet::MagneticFieldGrid* grid, double& z_in, // z value for input parameters
    double*       p_in,                                        // input track parameters (x,y,tx,ty,Q/p)
    const double& error,                                       // acceptable errors in mm
    double&       z_out,                                       // z value for output parameters
...

{
  ...
  rk5fast( grid, z_in, p_in, error, z_out, p_out );
  ...
  for ( j = 0; j < 5; ++j ) {
    ...
    for ( i = 0; i < 5; ++i ) { p1_in[i] = p_in[i]; } // i

    p1_in[j] += d_p[j];
    rk5fast( grid, z_in, p1_in, error, z_out, p1_out );

    for ( i = 0; i < 5; ++i ) { rkd[5 * j + i] = ( p1_out[i] - p_out[i] ) / d_p[j]; } // i
  }                                                                                   // j
...
//===========================================================================
void TrackHerabExtrapolator::rk4order( const LHCb::Magnet::MagneticFieldGrid* grid,
                                       double&                                z_in, // z value for input parameters
                                       double* p_in,       // input track parameters (x,y,tx,ty,Q/p)
                                       double& z_out,      // z value for output parameters
                                       double* p_out,      // output track parameters
...

{
  ...
  for ( step = 0; step < 4; ++step ) {
    ...
    Ax_tx[step] = Ax[step] * tx * I_tx2ty21 + ( ty * mB.x() - 2.0 * tx * mB.y() ) * tx2ty2qp;
    Ax_ty[step] = Ax[step] * ty * I_tx2ty21 + ( tx * mB.x() + mB.z() ) * tx2ty2qp;
    Ay_tx[step] = Ay[step] * tx * I_tx2ty21 + ( -ty * mB.y() - mB.z() ) * tx2ty2qp;
    Ay_ty[step] = Ay[step] * ty * I_tx2ty21 + ( -tx * mB.y() + 2.0 * ty * mB.x() ) * tx2ty2qp;
  ...
  for ( i = 0; i < 4; ++i ) { p_out[i] = x0[i] + c[0] * k[i] + c[1] * k[4 + i] + c[2] * k[8 + i] + c[3] * k[12 + i]; }
  p_out[4] = p_in[4];
  //
  //     Derivatives    dx/dqp
  //
  ...
  for ( i = 0; i < 4; ++i ) {
    if ( i != 2 ) { rkd[10 + i] = x0[i] + c[0] * k1[i] + c[1] * k1[4 + i] + c[2] * k1[8 + i] + c[3] * k1[12 + i]; }
  }
  //      end of derivatives dx/dtx
  rkd[12] = 1.0;
  rkd[14] = 0.0;
...
//===========================================================================
void TrackHerabExtrapolator::rk4fast( const LHCb::Magnet::MagneticFieldGrid* grid,
                                      double&                                z_in, // z value for input parameters
                                      double* p_in,       // input track parameters (x,y,tx,ty,Q/p)
                                      double& z_out,      // z value for output parameters
                                      double* p_out,      // output track parameters
