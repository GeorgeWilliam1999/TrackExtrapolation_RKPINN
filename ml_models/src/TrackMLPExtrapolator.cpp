...
// Local
#include "../../src/TrackFieldExtrapolatorBase.h"
...
namespace {
  ...
  inline Eigen::VectorXd silu( const Eigen::VectorXd& x ) {
  ...
  public:
    ...
    bool isLoaded = false;
    bool isResidual = false;  // PINN residual mode: output = input[:4] + correction
    ...
    Eigen::VectorXd forward( const Eigen::VectorXd& input ) const {
      ...
      // Note: For PINN residual models, the network already computes 
      // output = IC + correction in normalized space. After denormalization,
      // we have the final output directly. No need to add IC again.
      // The isResidual flag is just for informational purposes.
    ...
    bool load( const std::string& filepath ) {
      ...
      // Read first integer - could be model_type (0=MLP, 1=PINN) or num_layers
      // V1 format: first int is num_layers (typically 2-5)
      // V2 format: first int is model_type (0 or 1), second is num_layers
      int first_int;
      ...
      // Heuristic: if first_int is 0 or 1, it's the new format with model_type header
      // If first_int > 1, it's the old format where this is num_layers directly
      if ( first_int <= 1 ) {
      ...
      // Read activation function (optional, for backward compatibility)
      int act_len;
      ...
      isLoaded = file.good() || file.eof();  // EOF is OK if we got all required data
      return isLoaded;
    }
